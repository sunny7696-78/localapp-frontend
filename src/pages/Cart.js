...
const Cart = () => {
  ...
  const submitOrder = async (paymentStatus) => {
    setPlacing(true);
    try {
      const res = await orderAPI.place({ type: cartType, items: cartItems.map(i => ({ itemId: i._id, name: i.name, price: i.price, quantity: i.qty, image: i.image })), deliveryAddress: address, restaurantId, paymentMethod: payment, paymentStatus, notes, discount });
      clearCart();
      ...
      {showUpiModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div className="card" style={{ background: 'white', borderRadius: 16, padding: 28, width: '90%', maxWidth: 380, textAlign: 'center' }}>
            <h3 style={{ marginBottom: 4 }}>📱 UPI se Pay Karo</h3>
            <p style={{ color: 'var(--muted)', fontSize: '0.85rem', marginBottom: 18 }}>PhonePe, GPay ya Paytm se QR scan karo</p>
            ...
            <a href={upiLink} style={{ display: 'block', textDecoration: 'none', marginBottom: 10 }}>
              <button className="btn btn-outline btn-block">Mobile mein UPI App Kholo</button>
            </a>

            <button className="btn btn-primary btn-block" onClick={confirmUpiPaid} disabled={upiConfirming}>
              {upiConfirming ? 'Confirm ho raha...' : '✅ Maine Pay Kar Diya'}
            </button>

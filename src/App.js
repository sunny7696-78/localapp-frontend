...
import './index.css';
const PrivateRoute = ({ children }) => { const { user, loading } = useAuth(); if (loading) return <div className="loader"><div className="spinner"/></div>; return user ? children : <Navigate to="/login" />; };
const AdminRoute = ({ children }) => { const { user, loading } = useAuth(); if (loading) return <div className="loader"><div className="spinner"/></div>; return user?.role==='admin' ? children : <Navigate to="/" />; };
const AppRoutes = () => {
  const { user } = useAuth();
  const HomeComponent = () => { if(user?.role==='driver') return <Navigate to="/driver"/>; if(user?.role==='vendor') return <Navigate to="/vendor"/>; return <Home/>; };
...
const App = () => (
  <BrowserRouter><AuthProvider><CartProvider>

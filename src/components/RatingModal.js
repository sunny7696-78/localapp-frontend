...
const RatingModal = ({ isOpen, onClose, targetType, targetId, targetName, orderId }) => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await reviewAPI.create({ targetType, targetId, rating, comment, orderId });
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}}>
      <div style={{background:'white',borderRadius:20,padding:28,width:'100%',maxWidth:400,boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}}>
        <h3 style={{marginBottom:6}}>⭐ Rate & Review</h3>
        ...
        <div style={{display:'flex',gap:8,justifyContent:'center',marginBottom:20}}>
          {[1,2,3,4,5].map(star=>(
            <span key={star} onClick={()=>setRating(star)} onMouseEnter={()=>setHover(star)} onMouseLeave={()=>setHover(0)}
              style={{fontSize:'2.5rem',cursor:'pointer',color:(hover||rating)>=star?'#f59e0b':'var(--border)',transition:'color 0.1s'}}>★</span>
          ))}
        </div>
        ...
        <div style={{display:'flex',gap:10}}>
          ...
          <button className="btn btn-primary" style={{flex:1}} onClick={handleSubmit} disabled={submitting}>{submitting?'Submitting...':'Submit Review'}</button>
        </div>
      </div>
    </div>
  );
